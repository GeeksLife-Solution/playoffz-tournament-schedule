        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="title text-start m-0">@lang('Court List')</h5>
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        // Group matches by play_area_id (court)
                        const courtGroups = {};
                        response.data.schedule.game_match.forEach(match => {
                            if (!courtGroups[match.play_area_id]) {
                                courtGroups[match.play_area_id] = {
                                    courtName: match.play_area_name,
                                    matches: []
                                };
                            }
                            courtGroups[match.play_area_id].matches.push(match);
                        });
                        
                        // Sort courts by name
                        const sortedCourts = Object.keys(courtGroups).sort((a, b) => {
                            return courtGroups[a].courtName.localeCompare(courtGroups[b].courtName);
                        });
                        
                        // Create accordion for each court
                        sortedCourts.forEach((courtId, index) => {
                            const court = courtGroups[courtId];
                            const isFirst = index === 0;
                            
                            html += `
                                <div class="accordion-item">
                                    <h2 class="accordion-header" id="headingCourt${courtId}">
                                        <button class="accordion-button ${isFirst ? '' : 'collapsed'}" type="button" data-bs-toggle="collapse" data-bs-target="#collapseCourt${courtId}" aria-expanded="${isFirst ? 'true' : 'false'}" aria-controls="collapseCourt${courtId}">
                                            <i class="fas fa-map-marker-alt me-2"></i>
                                            <strong>${court.courtName || 'Unassigned Court'}</strong>
                                            <span class="badge bg-secondary ms-2">${court.matches.length} matches</span>
                                        </button>
                                    </h2>
                                    <div id="collapseCourt${courtId}" class="accordion-collapse collapse ${isFirst ? 'show' : ''}" aria-labelledby="headingCourt${courtId}" data-bs-parent="#schedule-details">
                            ...
                            court.matches.forEach((match, matchIndex) => {
                                ...
                                html += `
                                    <tr>
                                        ...
                                        <td>${match.play_group_name || 'N/A'}</td>
